...
// Emscripten binding: takes JS array of locations, returns JS result object
emscripten::val optimizeRouteWASM(emscripten::val jsLocations) {
    try {
        int numLocations = jsLocations["length"].as<int>();
        
        if (numLocations < 2) {
            emscripten::val result = emscripten::val::object();
            result.set("status", emscripten::val("error"));
            result.set("error", emscripten::val("Need at least 2 locations"));
            return result;
        }
        ...
        for (int i = 0; i < numLocations; i++) {
            Location loc;
            loc.id = i;
            loc.lat = jsLocations[i]["lat"].as<double>();
            loc.lon = jsLocations[i]["lon"].as<double>();
            loc.type = jsLocations[i]["type"].as<string>();
            
            if (jsLocations[i].hasOwnProperty("isHighPriority")) {
                loc.isHighPriority = jsLocations[i]["isHighPriority"].as<bool>();
            } else {
        ...
        emscripten::val result = emscripten::val::object();
        result.set("status", emscripten::val("success"));
        result.set("message", emscripten::val("Route optimized with Hybrid Optimal Algorithm!"));
        result.set("total_locations", emscripten::val(numLocations));
        result.set("total_route_distance", emscripten::val(totalDistance));
        result.set("algorithm", emscripten::val(numLocations <= 12 ? "Held-Karp DP (OPTIMAL)" : "Christofides (1.5x Approx)"));
        
        emscripten::val jsRoute = emscripten::val::array();
        for (size_t i = 0; i < optimizedRoute.size(); i++) {
            emscripten::val locObj = emscripten::val::object();
            locObj.set("id", emscripten::val(optimizedRoute[i].id));
            locObj.set("type", emscripten::val(optimizedRoute[i].type));
            locObj.set("lat", emscripten::val(optimizedRoute[i].lat));
            locObj.set("lon", emscripten::val(optimizedRoute[i].lon));
            locObj.set("isHighPriority", emscripten::val(optimizedRoute[i].isHighPriority));
            locObj.set("eta_minutes", emscripten::val(optimizedRoute[i].eta));
            ...
            locObj.set("eta_formatted", emscripten::val(string(etaStr)));
            
            jsRoute.call<void>("push", locObj);
        }
        result.set("optimized_route", jsRoute);
        ...
        emscripten::val result = emscripten::val::object();
        result.set("status", emscripten::val("error"));
        result.set("error", emscripten::val(string("Optimization failed: ") + e.what()));
        return result;
    }
}

EMSCRIPTEN_BINDINGS(postai_module) {
    emscripten::function("optimizeRoute", &optimizeRouteWASM);
}
